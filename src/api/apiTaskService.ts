...
export async function getTasksSorted(sort: "asc" | "desc"): Promise<Task[]> {
  try {
    const res = await fetch(`${BASE_URL}/tasks?sort=${sort}`);
    if (!res.ok) throw new Error("Error sorting tasks");
    const data: Task[] = await res.json();
    return data;
  } catch (err) {
    console.error(err);
...
// GET /tasks/stats
export async function getTaskStats(): Promise<{ total: number; done: number; pending: number }> {
  try {
    const res = await fetch(`${BASE_URL}/tasks/stats`);
    if (!res.ok) throw new Error("Error fetching task stats");
    const data: { total: number; done: number; pending: number } = await res.json();
    return data;
  } catch (err) {
    console.error(err);
    throw err;
...
// PATCH /tasks/:id
export async function patchTask(id: number, data: Partial<Task>): Promise<Task> {
  try {
    const res = await fetch(`${BASE_URL}/tasks/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    if (!res.ok) throw new Error("Error patching task");
...
export async function getCommentsSorted(taskId: number, sort: "newest" | "oldest"): Promise<Comment[]> {
  try {
    const res = await fetch(`${BASE_URL}/tasks/${taskId}/comments?sort=${sort}`);
    if (!res.ok) throw new Error("Error sorting comments");
    const data: Comment[] = await res.json();
    return data;
  } catch (err) {
    console.error(err);
...
export async function updateComment(taskId: number, commentId: number, data: { content?: string; resolved?: boolean }): Promise<Comment> {
  try {
